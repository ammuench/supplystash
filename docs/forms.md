...
- **Schemas live in `lib/schemas/`**, one file for each domain (`lib/schemas/auth.ts`). The schema
  holds the messages that the user reads, thus the screen contains no message text.
...
- **Show field errors with `<FormFieldErrors errors={field.state.meta.errors} />**
  (`components/form-field-errors.tsx`). Standard Schema supplies issue objects, not strings. The
  component extracts the messages and removes the duplicates that the two validators cause.
...
Supabase (`AuthFailure` from `lib/auth.ts`) show at form level, because a rejected credential pair
does not belong to one field.

## The password policy

`lib/schemas/auth.ts` holds the policy and mirrors `supabase/config.toml` → `[auth]`. Change the two
files together. Sign-in makes a check only for a value that is not empty: a stricter check would
prevent access for an account that an earlier policy created.
